...
class LeNet(Module_Base):
    ...
    def __init__(self, conv1_kernel_num_i = 6, conv2_kernel_num_i = 16):
        ...
        self.conv1_activation_func = F.relu
        self.conv2_activation_func = F.relu
        self.fc1_activation_func = F.relu
        self.fc2_activation_func = F.relu
        self.fc3_activation_func = F.relu
    ...
    def change_activation_function(self):
        ...
        if p < 0.2:
            # change conv1
            self.conv1_activation_func = torch.tanh
        elif p < 0.4:
            # change conv2
            self.conv2_activation_func = torch.tanh
        elif p < 0.6:
            # change fc1
            self.fc1_activation_func = torch.tanh
        elif p < 0.8:
            # change fc2
            self.fc2_activation_func = torch.tanh
        else:
            # change fc3
            self.fc3_activation_func = torch.tanh
