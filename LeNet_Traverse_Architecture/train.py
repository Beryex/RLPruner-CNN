...
cur_batch_window_opts = {
    'title': 'Model score growth trend',
    'xlabel': 'Model Number',
    'ylabel': 'Model Score',
    'width': 1200,
    'height':10000,
...
max_conv2_kernel_num = 50
max_training_times = 1000
...
def train_weights_bias(model_architecture, model_training_result, training_id):
    ...
    cur_model_score = compute_score(final_average_accuracies, epoches_average_needed, model_architecture)
...
def compute_score(final_average_accuracies, epoches_average_needed, model_architecture):
    cur_model_score = final_average_accuracies * 1000 - epoches_average_needed * model_architecture[0] * model_architecture[1] / 2 / 10
    return cur_model_score
...
def update_model_architecture(model_training_result):
    ...
    cur_score = model_training_result[cur_conv1_kernel_num][cur_conv2_kernel_num]['score']
    prev_score = model_training_result[cur_conv1_kernel_num][cur_conv2_kernel_num - 1]['score']
    next_score = model_training_result[cur_conv1_kernel_num][cur_conv2_kernel_num + 1]['score']
    ...
    if prev_score == 0 and next_score == 0:
        ...
        return 0
    if max(prev_score, next_score) > cur_score:
        if next_score >= prev_score:
            LeNet_Module.conv2_kernel_num += 1
            if LeNet_Module.conv2_kernel_num > LeNet_Module.conv1_kernel_num * 3.8:
                LeNet_Module.conv1_kernel_num +=1
            if model_training_result[LeNet_Module.conv1_kernel_num][LeNet_Module.conv2_kernel_num]['score'] != 0:
                return update_model_architecture(model_training_result)
            return 0
        if prev_score > next_score:
            ...
            if model_training_result[LeNet_Module.conv1_kernel_num][LeNet_Module.conv2_kernel_num]['score'] != 0:
                return update_model_architecture(model_training_result)
            return 0
    if min(prev_score, next_score) == 0:
        if next_score == 0:
            LeNet_Module.conv2_kernel_num += 1
            if LeNet_Module.conv2_kernel_num > LeNet_Module.conv1_kernel_num * 3.8:
                LeNet_Module.conv1_kernel_num +=1
...
def traverse_architecture():
    cur_conv1_kernel_num = LeNet_Module.conv1_kernel_num
    cur_conv2_kernel_num = LeNet_Module.conv2_kernel_num
    if cur_conv2_kernel_num >= max_conv2_kernel_num or cur_conv1_kernel_num >= max_conv1_kernel_num:
        print('Traverse architecture finished')
        return 1
    else:
        LeNet_Module.conv2_kernel_num += 1
        if LeNet_Module.conv2_kernel_num > LeNet_Module.conv1_kernel_num * 3.8:
...
if __name__ == '__main__':
    ...
    prev_model_score = 0
    best_model_score = -1
    best_architecture = np.array([LeNet_Module.conv1_kernel_num, LeNet_Module.conv2_kernel_num])
    model_scores = []
    model_list = []
    for training_id in range(max_training_times):
        model_list.append(training_id + 1)
        ...
        model_scores.append(cur_model_score)
        if cur_model_score > best_model_score:
            best_model_score = cur_model_score
            best_architecture[0] = LeNet_Module.conv1_kernel_num
            best_architecture[1] = LeNet_Module.conv2_kernel_num
        if traverse_architecture() == 1:
            print('The local optimal architure has %f conv1_kernel_num, %f conv2_kernel_num with score %f' %(best_architecture[0], best_architecture[1], best_model_score))
            break
        '''
        # if score converge, stop training
        if np.abs(cur_model_score - prev_model_score) < 1:
            print('The local optimal architure has %f conv1_kernel_num, %f conv2_kernel_num' %(LeNet_Module.conv1_kernel_num, LeNet_Module.conv2_kernel_num))
            break
    ...
    # visualization this training final accuracy
    if viz.check_connection():
        cur_batch_window = viz.line(torch.Tensor(model_scores), torch.Tensor(model_list),
                            win=cur_batch_window, name='model score growth trend',
                            update=(None if cur_batch_window is None else 'append'),
                            opts=cur_batch_window_opts)
